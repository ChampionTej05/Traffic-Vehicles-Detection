...
def draw_bounding_box_on_image(current_frame_number,image,
  ...
  if(left > ROI_POSITION): # if the vehicle get in ROI area, vehicle predicted_speed predicted_color algorithms are called - 200 is an arbitrary value, for my case it looks very well to set position of ROI line at y pixel 200
        print("")
...
def visualize_boxes_and_labels_on_image_array(current_frame_number,image,
  ...
  for box, color in box_to_color_map.items():
    ...
    if ( ("person" in display_str_list[0]) or ("motorcycle" in display_str_list[0]) or ("car" in display_str_list[0]) or ("truck" in display_str_list[0]) or ("bus" in display_str_list[0])):
